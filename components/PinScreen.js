...
function PinScreen(props) {
    ...
    const {mode , sid, cid, token, url, beacon_id} = props.navigation.state.params;  // navigation.getParam('') ?
    ...
    function confirmPin() {
        ...
        if(savedPin == pin) {
            // 핀번호 일치 (문열기)
            open_door( {sid:sid, cid: cid, token:token, url:url, beacon_id:beacon_id } );
        } else {
